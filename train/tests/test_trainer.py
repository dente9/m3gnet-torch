# m3gnet/train/tests/test_trainer.py (Final Keyword Argument Fixed Version)
...
class TestTrainers(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        print(f"Loaded {len(cls.structures)} structures from {DATA_DIR}")

        cls.property_targets = np.random.rand(len(cls.structures)) * 5
        ...
        cls.potential_energies = np.random.rand(len(cls.structures)) * -10
    ...
    def test_01_property_trainer(self):
        ...
        try:
            trainer.train(
                train_structures=self.train_structs, train_targets=train_targets,
                val_structures=self.val_structs, val_targets=val_targets,
    ...
    def test_02_potential_trainer(self):
        ...
        try:
            trainer.train(
                train_structures=self.train_structs, train_energies=train_energies,
                train_forces=train_forces, train_stresses=train_stresses,
                val_structures=self.val_structs, val_energies=val_energies,
    ...
    def test_03_callbacks(self):
        ...
        try:
            with self.subTest(callback="ModelCheckpoint"), unittest.mock.patch('builtins.print'):
                ...
                # Check if both best and last model files were created
